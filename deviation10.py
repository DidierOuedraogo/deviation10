...
<style>
    ...
    /* Correction des textes des boutons radio */
    ...
    /* Correction spécifique pour les textes de sidebar */
    .sidebar-heading {
        color: white !important;
    ...
    /* Styling pour le champ de nom du forage */
    .drill-name-input input {
    ...
    .drill-name-input input:focus {
        border-color: #3563E9 !important;
        box-shadow: 0 0 0 2px rgba(53, 99, 233, 0.2) !important;
    }
    
    /* Étiquette avec couleur de fond pour le nom du forage */
    .drill-name-badge {
        display: inline-block;
        background-color: #EFF6FF;
        color: #1E40AF;
...
with st.sidebar:
    ...
    st.markdown('<p class="sidebar-heading" style="color: white;">Source des données</p>', unsafe_allow_html=True)
    ...
    # Sélection du modèle
    st.markdown('<p class="sidebar-heading" style="color: white;">Modèle de machine learning</p>', unsafe_allow_html=True)
    ...
    st.markdown('<p class="sidebar-heading" style="color: white;">Augmentation des données</p>', unsafe_allow_html=True)
...
if df is not None:
    ...
    with tabs[2]:  # Prédiction
        ...
        with col1:
            ...
            # Nouveau champ pour le nom du forage
            drill_name = st.text_input("Nom du forage", value="FOR-001", help="Identifiant ou nom du forage", key="drill_name_input", 
                                      placeholder="ex: FOR-001, MINE-A-123, etc.", max_chars=50)
            ...
            # Afficher le nom du forage sous forme de badge
            st.markdown(f"""
            <div class="drill-name-badge">
                <span>Forage: {drill_name}</span>
            </div>
            """, unsafe_allow_html=True)
        ...
        if predict_button and st.session_state.model_trained:
            ...
            <div style="background-color: white; border-radius: 12px; box-shadow: 0 4px 16px rgba(0, 0, 0, 0.08); padding: 1.75rem; margin: 1.75rem 0; border: 1px solid #f0f0f5;">
                <h4 style="margin-top: 0; text-align: center; margin-bottom: 1.75rem; color: #1e3a8a; font-family: 'Poppins', sans-serif;">Prédiction pour le forage: {drill_name}</h4>
            """, unsafe_allow_html=True)
            ...
            fig.update_layout(
                title=f"Trajectoire du forage {drill_name} (Écart final: {final_deviation:.2f} m)",
                scene=dict(
                    xaxis_title='Est (m)',
                    yaxis_title='Nord (m)',
                    zaxis_title='Profondeur (m)',
            ...
            with col1:
                ...
                <div style="background-color: white; border-radius: 12px; box-shadow: 0 4px 16px rgba(0, 0, 0, 0.08); padding: 1.75rem; height: 100%; border: 1px solid #f0f0f5;">
                    <h4 style="margin-top: 0; color: #1e3a8a; font-family: 'Poppins', sans-serif;">Recommandations pour {drill_name}</h4>
...
Didier Ouedraogo, P.Geo. - Géologue & Data Scientist
            ...
            with col2:
                st.download_button(
                    label=f"📄 Télécharger le rapport pour {drill_name}",
                    data=report_text,
                    file_name=f"rapport_deviation_forage_{drill_name}_{datetime.datetime.now().strftime('%Y%m%d_%H%M%S')}.txt",
                    mime="text/plain",
